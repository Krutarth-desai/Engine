      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
          ...
          <span style={{ fontSize: "0.7rem", color: "var(--text-muted, #64748b)" }}>
            | Continuous 1 Hz propulsion telemetry feed
          </span>
        </div>

        <div style={{ fontSize: "0.7rem", color: "var(--text-secondary, #94a3b8)" }}>
          To inspect a previous sortie, select a mission in <strong>MISSION HISTORY</strong>.
        </div>
      </div>
    );
  }
    ...
    >
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "0.5rem" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
          {/* Play/Pause Toggle */}
          <button
            className={`gcs-btn gcs-btn-sm ${isPaused ? "gcs-btn-primary" : "gcs-btn-secondary"}`}
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "0.25rem" }}>
            {SPEED_OPTIONS.map((spd) => (
              <button
                key={spd}
                className={`gcs-btn gcs-btn-sm ${currentSpeed === spd ? "gcs-btn-primary" : "gcs-btn-ghost"}`}
                style={{ padding: "0 0.4rem", height: "24px" }}
          ...
          <button
            className="gcs-btn gcs-btn-sm gcs-btn-danger"
            onClick={() => stopReplay()}
          >
            ⏹ RETURN TO LIVE
          </button>
